...
function loadMenu() {
  loadContent('menus/menu');
}

function loadAuth() {
  loadContent('auth/index');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  addMenuButton.addEventListener('click', loadMenu);

    // '로그인' 버튼에 대한 이벤트 리스너 등록
  const loginButton = document.getElementById('login-button');
  loginButton.addEventListener('click', loadAuth);
